...
@interface MXPopupMenuCell : UITableViewCell
@property (nonatomic, assign) BOOL isShowSeparator;
@property (nonatomic, strong) UIColor * separatorColor;
@end

@implementation MXPopupMenuCell
...
+ (instancetype)showAtPoint:(CGPoint)point titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<MXPopupMenuDelegate>)delegate
{
    MXPopupMenu *popupMenu = [[MXPopupMenu alloc] initWithTitles:titles icons:icons menuWidth:itemWidth delegate:delegate];
    [popupMenu showAtPoint:point];
    return popupMenu;
}

+ (instancetype)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<MXPopupMenuDelegate>)delegate
{
    MXPopupMenu *popupMenu = [[MXPopupMenu alloc] initWithTitles:titles icons:icons menuWidth:itemWidth delegate:delegate];
...
{
    static NSString * identifier = @"MXPopupMenu";
    MXPopupMenuCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[MXPopupMenuCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:identifier];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(mx_PopupMenuDidSelectedAtIndex:popupMenu:)]) {
        
        [self.delegate mx_PopupMenuDidSelectedAtIndex:indexPath.row popupMenu:self];
    }
}
...
{
    MXPopupMenuCell *cell = [self getLastVisibleCell];
...
- (MXPopupMenuCell *)getLastVisibleCell
{
...
- (void)setType:(MXPopupMenuType)type
{
    _type = type;
    switch (type) {
        case MXPopupMenuTypeDark:
        {
